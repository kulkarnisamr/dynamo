...
func TestDynamoGraphDeploymentValidator_Validate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "GMS snapshot combination requires feature gate",
			deployment: betaDGDForAdmission(func(dgd *nvidiacomv1beta1.DynamoGraphDeployment) {
				worker := betaWorkerComponent(dgd)
				enableBetaIntraPodGMS(worker)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := features.WithGates(dgdAdmissionContextWithUserInfo(
				...
				tt.userInfo,
			), features.Gates{Grove: !tt.groveDisabled})
			var (
				warnings []string
				err      error
			)
...
func TestDynamoGraphDeploymentConversionFailureIsFatal(t *testing.T) {
	...
	validator := newDynamoGraphDeploymentTestValidator(t, features.Gates{Grove: true})
...
func newDynamoGraphDeploymentTestValidator(t *testing.T, gates features.Gates) *DynamoGraphDeploymentValidator {
	t.Helper()
	return NewDynamoGraphDeploymentValidator(newGroveTopologyTestManager(t), gates)
}
